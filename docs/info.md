...
Implemented operations are `ACT`, `PRE`, `WR`, `RD`, `REF`, `STATUS`, `CONFIG`, `ABORT`, `NOP`, `VXOR`, `VAND`, `VOR`, `VADD`, `VSUB`, `DOT`, `MAC`, `SUM`, `POPCNT`, `XNORDOT`, and accumulator byte reads. `DOT` and `MAC` use unsigned INT1 bit semantics or signed INT2/INT4/INT8 lane semantics and are computed through a lane-serial accumulator datapath.

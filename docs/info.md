...
It exposes a 32-bit SPI command interface and implements two logical channels. Each channel has two banks, four 8-bit rows per bank, configurable refresh state with readback and auto-refresh enable control, an 18-bit accumulator, one pending command slot, and a lane-serial PIM datapath.

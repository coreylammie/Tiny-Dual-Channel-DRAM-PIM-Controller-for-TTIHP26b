...
| 2026-09-13 | `PATH=/private/tmp/tto-cocotb-venv2/bin:/private/tmp/tto-kvupd-venv/bin:$PATH make -C test` | PASS, 17 TinyTapeout-wrapper cocotb RTL tests on coverage branch `cocotb-coverage-gaps`. Added refresh-bank blocking, invalid encoded-field handling, other-channel local-command interleaving while the shared PU is busy, back-to-back SPI command stress, and randomized model-equivalence coverage for control/memory operations. |

...
`STREAM.DOT` and `STREAM.MAC` reuse that same lane-serial engine across consecutive row pairs. `imm8[2:0]` selects a count from 1 through 4; count 0 is invalid. `STREAM.DOT` clears the accumulator once before the first row pair. `STREAM.MAC` keeps the previous accumulator value.
